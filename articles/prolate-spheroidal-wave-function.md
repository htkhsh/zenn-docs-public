...
行列 $A^{\text {even }}$ と $A^{\text {odd }}$ は無限行列であり、その成分は行番号や列番号の増加とともに減衰しないが、各固有ベクトル $\beta^{(n)}$ の各成分は超指数的に減衰することが知られている。

## 固有値の評価

上記のPSWF評価アルゴリズムは微分演算子(24)の固有値 $\left\{\chi_j\right\}$ も生成するが、積分演算子 $F_c$ の固有値 $\left\{\lambda_j\right\}$ は生成しない。これらの固有値の一部は以下の式を用いて計算できる：
...
1. $m=0$ に対する固有値 $\lambda_0$ を以下の式を用いて数値的に評価する：
   $$
   \lambda_m=\frac{1}{\psi_m(0)} \int_{-1}^1 \psi_m(t) d t
   $$

2. 計算された $\lambda_0$ と式
   $$
   \frac{\lambda_m^2}{\lambda_n^2}=\frac{\int_{-1}^1 \psi_n^{\prime}(x) \psi_m(x) d x}{\int_{-1}^1 \psi_m^{\prime}(x) \psi_n(x) d x}
   $$
   を用いて、$j=1,2, \ldots, m$ に対する絶対値 $\left|\lambda_j\right|$ を計算する。そして、各 $\left|\lambda_j\right|$ を $\left|\lambda_{j-1}\right|$ から計算する。

3. $\lambda_j=\mathrm{i}^j\left|\lambda_j\right|$ という事実（定理2.4を参照）を用いて計算を完成させる。
...
**定理3** $c>0$ を実数、$n \geq 0$ を整数とする。また、$x>1$ を実数とし、実数 $A_n^c(x)$ が以下の式で定義されるとする：

$$
A_n^c(x)=\frac{1}{\lambda_n(c) \psi_n^c(1)} \int_{-1}^1 \frac{\sin (c(x-t)) \psi_n^c(t) t}{x-t} d t
$$

するとPSWF $\psi_n^c(x)$ は、$n$ が偶数の場合、

$$
\psi_n^c(x)=\frac{2 \psi_n^c(1)}{c x \lambda_n(c)}\left[\sin (c x)+A_n^c(x)\right]
$$

$n$ が奇数の場合、

$$
\psi_n^c(x)=-\frac{2 \psi_n^c(1)}{c x \lambda_n(c)}\left[i \cos (c x)+A_n^c(x)\right]
$$

と表せられる。特に、

$$
\left|A_n^c(x)\right| \leq \frac{1}{\left|\lambda_n(c) \cdot \psi_n^c(1)\right| \cdot(x-1)}
$$

が成り立つ。

以上から分かるように、区間 $|x|>1$ において、PSWFの振幅は必ず減衰する。この性質は、　$t \rightarrow \infty$ で $0$ に収束する関数を近似するのに適していることを示している。
